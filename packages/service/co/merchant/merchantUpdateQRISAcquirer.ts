...
import { CooperationAgreement } from "./merchantCreateQRISAcquirer";

export interface MerchantUpdateQRISAcquirerResponse extends ResponseData { }

export interface MerchantUpdateQRISAcquirerRequest extends DefaultRequest {
  accountName?: string;
...
function useMerchantUpdateQRISAcquirerFetcher(baseUrl: string, id: string, payload: MerchantUpdateQRISAcquirerRequest) {
  return apiPut<MerchantUpdateQRISAcquirerResponse>({
    baseUrl,
    path: `${apiMerchant}/${id}`,
    payload,
  });
...
export default useMerchantUpdateQRISAcquirerFetcher;
