...
const CreateRoleModal = (props: CreateRoleModalProps) => {
  const { selectedData, isActive, onHide, fetchRoleList } = props;
  const {
    userList,
    menuPrivileges,
    menuList,
            ...
            <Stack direction="column" key="roleMenuId">
              <FormControlLabel
                ...
                onChange={(_, checked) =>
                  setValue(
                    'menuPrivileges',
                    getValues('menuPrivileges').map(data => {
                      if (data.menuId === row.original.menuId) {
      ...
      <DialogTitle>
        ...
        >
          <Typography variant="h5">
            {isUpdate ? tRole('modalUpdateTitle') : tRole('modalCreateTitle')}
          </Typography>
        ...
        <Grid container spacing={2} sx={{ pt: 1 }}>
          ...
          <Grid item md={12} xs={12} sx={{ py: 2 }}>
            <Card sx={{ p: 2, borderRadius: 4 }}>
              ...
              {touched &&
                !getValues('menuPrivileges').some(
                  menuPrivilege =>
                    menuPrivilege.checked && menuPrivilege.privilegeType,
                ) && (
                  <FormHelperText sx={{ color: Token.color.redDark }}>
          ...
          {isUpdate && (
            <React.Fragment>
              <Grid item md={12} xs={12}>
                <Typography variant="subtitle1" gutterBottom>
                  {tRole('listOfUser', { number: userList.length })}
                </Typography>
                <TextField
                  value={searchUser}
      ...
      <DialogActions>
        ...
        >
          {isUpdate && (
            ...
            >
              {getValues('isActive')
                ? tRole('actionDectivate')
                : tRole('actionActivate')}
            </Button>
          )}
