...
// Hooks & Utils
import { useTransactionTypeOptionListFetcher, useAccountRuleValueListFetcher, useAccountRuleOptionListFetcher } from "@woi/service/co";
...
import { AccountRuleValueData, AccountRuleValueListRequest } from "@woi/service/co/admin/accountRuleValue/accountRuleValueList";
import { OptionMap } from "@woi/option";
import { DatePeriod } from "@woi/core/utils/date/types";

type FilterForm = {
  accountRuleSecureIds: OptionMap<string>[];
  transactionTypeSecureIds: OptionMap<string>[];
...
function useAccountRuleValueList() {
  ...
  const accountRuleValueListPayload: AccountRuleValueListRequest = {
    ...
    accountRuleSecureIds: debouncedFilter.accountRuleSecureIds.map(data => data.value),
    transactionTypeSecureIds: debouncedFilter.transactionTypeSecureIds.map(data => data.value),
  ...
  const {
    ...
    async () => useAccountRuleValueListFetcher(baseUrl, accountRuleValueListPayload),
    {
      refetchOnWindowFocus: false,
      onSuccess: (response) => {
