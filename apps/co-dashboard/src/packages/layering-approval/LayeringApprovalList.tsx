...
const LayeringApprovalList = () => {
  ...
  const [selectedApprovalLayer, setSelectedApprovalLayer] =
    useState<ApprovalLayerData | null>(null);
  const {
    filterForm,
    setFilterForm,
    pagination,
  ...
  const columns: Array<Column<ApprovalLayerData & { action: string }>> =
    useMemo(
      () => [
        {
    ...
    <Stack direction="column">
      <Stack direction="column" spacing={2} sx={{ pb: 3 }}>
        ...
        >
          ...
          <AuthorizeView access="approval-layer" privileges={['create']}>
            <Button
              variant="contained"
              color="primary"
        ...
        {approvalLayerStatus === 'success' &&
          approvalLayerData.length === 0 && (
            <EmptyList
              ...
              >
                ...
                >
                  {tCommon('paginationTitle', {
                    totalShowing: approvalLayerData.length,
      ...
      {isActive && (
        <CreateLayeringApprovalModal
          isActive={isActive}
          onHide={hideModal}
          selectedData={selectedApprovalLayer}
          fetchApprovalLayerList={fetchApprovalLayerList}
        />
      )}
    </Stack>
  );
