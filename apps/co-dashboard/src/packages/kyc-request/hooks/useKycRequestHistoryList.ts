...
import { KycPremiumMemberHistoryData, KycPremiumMemberHistoryListRequest } from "@woi/service/co/kyc/premiumMember/premiumMemberHistoryList";
...
function useKycRequestHistoryList() {
  ...
  const kycRequestHistoryPayload: KycPremiumMemberHistoryListRequest = {
  ...
  const {
    ...
    status: kycRequestHistoryStatus,
    refetch: refetchKycRequest
  } = useQuery(
    ['kyc-request-history-list', kycRequestHistoryPayload],
    async () => useKycPremiumMemberHistoryListFetcher(baseUrl, kycRequestHistoryPayload),
    {
  ...
  const handleDownload = async () => {
    ...
    if (result && !error) {
      ...
      enqueueSnackbar(errorData?.details?.[0] || tCO('downloadKYCHistoryFailed'), { variant: 'error' });
    }
  };
  ...
  return {
    ...
    handleSort,
    handleDownload,
    kycRequestHistoryData: kycRequestHistoryData?.result?.data || [],
