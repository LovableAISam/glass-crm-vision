...
import { SMSContentData, SMSContentListRequest } from "@woi/service/co/admin/smsContent/smsContentList";
...
function useSMSContentList() {
  ...
  const getSortPayload = (paramSortBy: keyof SMSContentData) => {
    const sortMap: Record<keyof SMSContentData, string> = {
      subject: 'subject',
  ...
  const {
    ...
    async () => useSMSContentListFetcher(baseUrl, smsContentPayload),
    {
