...
import { SMSContentData } from '@woi/service/principal/admin/smsContent/smsContentList';
...
const CreateSMSContentManagementModal = (props: CreateSMSContentManagementModalProps) => {
  ...
  const { field: fieldSMSContentType } = useController({
    ...
    control,
    rules: {
      required: tForm('generalErrorRequired', { fieldName: 'SMS content type' }),
    }
  });
    ...
    >
      <DialogTitle>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography variant="h5">{isUpdate ? tSMSContent('modalUpdateTitle') : tSMSContent('modalCreateTitle')}</Typography>
        ...
        <Grid container spacing={2} sx={{ pt: 1 }}>
          <Grid item md={6} xs={12}>
            <Grid container spacing={2}>
              <Grid item md={12} xs={12}>
                <Typography variant="subtitle2" gutterBottom>{tSMSContent('formSMSSubject')}</Typography>
                <TextField
                  {...fieldSubject}
                  fullWidth
                  placeholder={tForm('placeholderType', { fieldName: 'sms subject' })}
                  sx={{
              ...
              <Grid item md={12} xs={12}>
                <Typography variant="subtitle2" gutterBottom>{tSMSContent('formSMSContent')}</Typography>
                <Card elevation={0} sx={{ p: 1, pr: 2, borderRadius: 3, mb: 1, backgroundColor: theme.palette.secondary.main }}>
                  <Typography variant="body2">
                    <ul>
                      <li>{tSMSContent('infoSMSContentTitle')}</li>
                      <li>
                        <div dangerouslySetInnerHTML={{ __html: tSMSContent('infoSMSContentDescription') }} />
                      </li>
                    </ul>
                  </Typography>
                </Card>
              ...
              <Grid item md={12} xs={12}>
                <Typography variant="subtitle2" gutterBottom>{tSMSContent('formSMSContentType')}</Typography>
                <Autocomplete
                  {...fieldSMSContentType}
          ...
          <Grid item md={6} xs={12}>
            <Card elevation={0} sx={{ background: Token.color.dashboardLightest, p: 3, borderRadius: 4, height: '100%' }}>
              <Typography variant="h5" sx={{ mb: 2, fontWeight: 'bold' }}>{tSMSContent('preview')}</Typography>
              <Stack direction="row" spacing={2}>
                <Avatar sx={{ backgroundColor: Token.color.secondaryBlueTintLightest, width: 48, height: 48 }}>
                  <PersonOutlineIcon sx={{ color: Token.color.primaryBlue, width: 32, height: 32 }} />
                </Avatar>
                <Card sx={{ p: 3, borderRadius: 4 }}>
                  <Stack direction="column" spacing={2}>
                    <Typography variant="h5" sx={{ fontWeight: 'bold' }}>{getValues('subject') || tSMSContent('placeholderSMSSubject')}</Typography>
                    {getValues('content') ? (
                      <Markdown
                        typographyProps={{
                          variant: 'body2',
                      ...
                      <Typography variant="body2">{tSMSContent('placeholderSMSContent')}</Typography>
                    )}
                  </Stack>
                </Card>
              </Stack>
        ...
        <Stack direction="row" alignItems="center" justifyContent={isUpdate ? 'space-between' : 'flex-end'} sx={{ p: 2, flex: 1 }}>
          {isUpdate && (
            <Stack direction="row" spacing={2}>
              ...
              >
                {getValues('isActive') ? tSMSContent('actionDectivate') : tSMSContent('actionActivate')}
              </Button>
              <Box>
