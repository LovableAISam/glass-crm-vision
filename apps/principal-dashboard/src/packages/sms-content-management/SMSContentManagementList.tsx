...
const SMSContentManagementList = () => {
  ...
  return (
    <Stack direction="column">
      <Stack direction="column" spacing={2} sx={{ pb: 3 }}>
        ...
        {(smsContentStatus === 'success' && smsContentData.length === 0) && (
          <EmptyList
            ...
            description={tCommon('tableEmptyDescription', { text: "SMS content" })}
          />
        )}
        {(smsContentStatus === 'success' && smsContentData.length > 0) && (
            ...
            <Stack direction="column" spacing={2}>
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center" justifyContent="space-between">
                <Typography variant="caption" color={Token.color.greyscaleGreyDarkest}>
                  {tCommon('paginationTitle', { totalShowing: smsContentData.length, totalData: pagination.totalElements })}
                </Typography>
                <Pagination
                  color="primary"
      ...
      {isActive && (
        <CreateSMSContentManagementModal selectedData={selectedSMS} isActive={isActive} onHide={hideModal} fetchSMSContentList={fetchSMSContentList} />
      )}
    </Stack>
  )
}
