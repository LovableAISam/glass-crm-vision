...
// Hooks & Utils
import { useRoleListFetcher, useUserDeleteFetcher, useUserListFetcher } from "@woi/service/principal";
...
import { UserData, UserListRequest } from "@woi/service/principal/idp/user/userList";
import { UserType } from "@woi/user";
...
function useUserList() {
  ...
  const { data: roleList } = useQuery(
    ['role-list'],
    async () => await useRoleListFetcher(baseUrl, {
      page: 0,
      limit: 1000,
    }),
  ...
  const {
    ...
    async () => await useUserListFetcher(baseUrl, userPayload),
    {
      refetchOnWindowFocus: false,
      onSuccess: (response) => {
