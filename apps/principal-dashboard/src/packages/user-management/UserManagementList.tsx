  ...
  const columns: Array<Column<UserData & { action: string }>> = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: 'role',
        Cell: ({ value, row }) => (
          <Typography variant="inherit" key="role" sx={{ color: row.original.isLocked ? Token.color.greyscaleGreyDarkest : Token.color.primaryBlack }}>{value}</Typography>
        )
      },
      {
        ...
        accessor: 'enabled',
        Cell: ({ value }) => {
          if (!value) return <Typography variant="inherit" color={Token.color.orangeDark}>{tUser('statusInactive')}</Typography>
          return <Typography variant="inherit" color={Token.color.greenDark}>{tUser('statusActive')}</Typography>
        }
      },
      {
    ...
    <Stack direction="column">
      <Stack direction="column" spacing={2} sx={{ pb: 3 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          ...
          <AuthorizeView access="principal" privileges={['create']}>
            <Button
              variant="contained"
              color="primary"
          ...
          <Grid container spacing={2}>
            <Grid item md={6} xs={12}>
              <Stack direction="column" spacing={1}>
                <Typography variant="subtitle2">{tUser('filterUsername')}</Typography>
                <TextField
                  ...
                  fullWidth
                  type="search"
                  placeholder={tForm('placeholderType', { fieldName: 'username' })}
                  sx={{
            ...
            <Grid item md={6} xs={12}>
              <Stack direction="column" spacing={1}>
                <Typography variant="subtitle2">{tUser('filterRole')}</Typography>
                <Autocomplete
                  value={roleOptions.filter(data => filterForm.role.some(filter => filter.value === data.value))}
                  onChange={(_, value) => {
                    batch(() => {
                      setPagination(oldPagination => ({
                        ...oldPagination,
            ...
            <Grid item md={6} xs={12}>
              <Stack direction="column" spacing={1}>
                <Typography variant="subtitle2">{tUser('filterStatus')}</Typography>
                <Autocomplete
            ...
            <Stack direction="column" spacing={2}>
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center" justifyContent="space-between">
                <Typography variant="caption" color={Token.color.greyscaleGreyDarkest}>
                  {tCommon('paginationTitle', { totalShowing: userData.length, totalData: pagination.totalElements })}
                </Typography>
                <Pagination
                  color="primary"
      ...
      {isActive && (
        <CreateUserModal isActive={isActive} onHide={hideModal} selectedData={selectedUser} fetchUserList={fetchUserList} />
      )}
    </Stack>
  )
}
