...
import { UserData } from '@woi/service/principal/idp/user/userList';
...
const CreateUserModal = (props: CreateUserModalProps) => {
  ...
  const { getValues, formState: { errors }, control } = formData;
  ...
  const { field: fieldUserEmail } = useController({
    ...
    control,
    rules: {
      required: isUpdate ? false : tForm('generalErrorRequired', { fieldName: 'Email' }),
      validate: value => {
  ...
  const { field: fieldPasswordConfirm } = useController({
    ...
    rules: isUpdate ? {} : {
      validate: value => {
        if (!value) return tForm('generalErrorRequired', { fieldName: 'Password Confirm' })
        ...
        >
          <Stack direction="column" spacing={1}>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between">
              <Typography variant="subtitle2" color={Token.color.greenDark}>{tUser('statusActive')}</Typography>
              <Button variant="outlined" size="small" onClick={handleActivateDeactivate} sx={{ py: 1, borderRadius: 2 }}>{tUser('actionDeactivate')}</Button>
            </Stack>
          </Stack>
        </Card>
      )
    ...
    return (
      ...
      >
        <Stack direction="column" spacing={1}>
          <Typography variant="body2">{tUser('detailUserStatus')}</Typography>
          <Stack direction="row" alignItems="center" justifyContent="space-between">
            <Typography variant="subtitle2" color={Token.color.redDark}>{tUser('statusInactive')}</Typography>
            <Button variant="outlined" size="small" onClick={handleActivateDeactivate} sx={{ py: 1, borderRadius: 2 }}>{tUser('actionActivate')}</Button>
          </Stack>
        </Stack>
      </Card>
    )
    ...
    if (getValues('isLocked')) {
      return (
        <Card sx={{ borderRadius: 3, p: 2, height: 100 }}>
          <Stack direction="column" spacing={1}>
            <Typography variant="body2" color={Token.color.greyscaleGreyDarkest}>{tUser('detailAccountStatus')}</Typography>
            <Stack direction="row" justifyContent="space-between" alignItems="center">
              <Typography variant="subtitle2" color={Token.color.redDark}>{tUser('statusLocked')}</Typography>
              <Button variant="outlined" size="small" sx={{ py: 1, borderRadius: 2 }} onClick={handleLockUnlock}>{tUser('actionUnlock')}</Button>
            </Stack>
          </Stack>
        </Card>
      )
    ...
    return (
      <Card sx={{ borderRadius: 3, p: 2, height: 100 }}>
        <Stack direction="column" spacing={1}>
          ...
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            <Typography variant="subtitle2" color={Token.color.greenDark}>{tUser('statusActive')}</Typography>
            <Button variant="outlined" size="small" sx={{ py: 1, borderRadius: 2 }} onClick={handleLockUnlock}>{tUser('actionLock')}</Button>
          </Stack>
        </Stack>
      </Card>
    )
  ...
  return (
    ...
    >
      <DialogTitle>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography variant="h5">{isUpdate ? tUser('modalUpdateTitle') : tUser('modalCreateTitle')}</Typography>
      ...
      <DialogContent>
        <Grid container spacing={2} sx={{ pt: 1 }}>
          {isUpdate ? (
              ...
              <Grid item md={4} xs={12}>
                ...
                >
                  <Stack direction="column" spacing={2}>
                    <Typography variant="body2">{tUser('detailEmail')}</Typography>
                    <Typography variant="subtitle2">{fieldUserEmail.value}</Typography>
                  </Stack>
                </Card>
              </Grid>
              <Grid item md={4} xs={12}>
            ...
            <React.Fragment>
              <Grid item md={12} xs={12}>
                <Typography variant="subtitle2" gutterBottom>{tUser('formEmail')}</Typography>
                <TextField
                  {...fieldUserEmail}
                  fullWidth
              ...
              <Grid item md={6} xs={12}>
                <Typography variant="subtitle2" gutterBottom>{tUser('formPassword')}</Typography>
                <PasswordInput
                  {...fieldPassword}
                  fullWidth
                ...
                />
                <Button variant="text" color="primary" onClick={handleGeneratePassword}>{tUser('actionGeneratePassword')}</Button>
              </Grid>
              <Grid item md={6} xs={12}>
                <Typography variant="subtitle2" gutterBottom>{tUser('formConfirmPassword')}</Typography>
                <PasswordInput
          ...
          <Grid item md={12} xs={12}>
            <Typography variant="subtitle2" gutterBottom>{tUser('formRole')}</Typography>
            <Autocomplete
              {...fieldRole}
          ...
          <Grid item md={12} xs={12}>
            <Typography variant="subtitle2" gutterBottom>{tUser('formDescription')}</Typography>
            <TextField
              {...fieldDescription}
              fullWidth
          ...
          <Grid item md={12} xs={12}>
            <Typography variant="subtitle2" gutterBottom>{tUser('formName')}</Typography>
            <TextField
              {...fieldName}
              fullWidth
          ...
          <Grid item md={12} xs={12}>
            <Typography variant="subtitle2" gutterBottom>{tUser('formEffectiveDate')}</Typography>
            <FormDatePicker
              {...fieldEffectiveDate}
