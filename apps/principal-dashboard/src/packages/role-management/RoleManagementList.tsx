...
const RoleManagementList = () => {
  ...
  return (
    <Stack direction="column">
      <Stack direction="column" spacing={2} sx={{ pb: 3 }}>
          ...
          <Grid container spacing={2}>
            <Grid item xl={4} md={6} xs={12}>
              <Stack direction="column" spacing={1}>
                <Typography variant="subtitle2">{tRole('filterRoleName')}</Typography>
                <TextField
                  value={searchRole}
        ...
        {(roleStatus === 'success' && roleData.length === 0) && (
          <EmptyList
            ...
            description={tCommon('tableEmptyDescription', { text: "role" })}
          />
        )}
        {(roleStatus === 'success' && roleData.length > 0) && (
            ...
            <Stack direction="column" spacing={2}>
              ...
              <Stack direction="row" alignItems="center" justifyContent="space-between">
                <Typography variant="caption" color={Token.color.greyscaleGreyDarkest}>
                  {tCommon('paginationTitle', { totalShowing: roleData.length, totalData: pagination.totalElements })}
                </Typography>
                <Pagination
                  color="primary"
      ...
      {isActive && (
        <CreateRoleModal selectedData={selectedRole} isActive={isActive} onHide={hideModal} fetchRoleList={fetchRoleList} />
      )}
    </Stack>
  )
}
