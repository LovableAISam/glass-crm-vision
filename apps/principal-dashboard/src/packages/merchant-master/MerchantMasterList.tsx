...
const MerchantMasterList = () => {
  const [sortBy, setSortBy] = useState<keyof MercantData>();
  ...
  const handleSort = (columnId: keyof MercantData) => {
    setSortBy(columnId);
    setDirection(oldDirection => reverseDirection(oldDirection));
  };

  const columns: Array<Column<MercantData & { action: string }>> = useMemo(
    () => [
      {
        Header: 'CO',
        accessor: 'co',
      ...
      },
      {
        ...
        Cell: () => (
          <Stack direction="row" spacing={2} key="merchantAction">
            <Button variant="text" size="small" onClick={showModalView}>
              View
            </Button>
            <Box>
  ...
  const data: MercantData[] = createDummy(10).map(() => ({
    co: 'WOI',
    merchantCode: 'Thu.30012081234567890',
    ...
    <Stack direction="column">
      <Stack direction="column" spacing={2} sx={{ pb: 3 }}>
        <Stack
          ...
          alignItems="center"
        >
          <Typography variant="h4">Merchant Master</Typography>
          <Button
            variant="contained"
            color="primary"
      ...
      <CreateMerchantModal isActive={isActive} onHide={hideModal} />
      <ViewMerchantModal isActive={isActiveView} onHide={hideModalView} />
    </Stack>
  );
};

export default MerchantMasterList;
