...
import { Box, Card, Stack, Drawer, List, Divider, Typography, useTheme } from '@mui/material';
...
const MenuNested = (menu: MenuType) => {
  ...
  useEffect(() => {
    if (menu && menu.children) {
      setOpen(menu.children.some(menuProps => router.pathname === menuProps.menuLink))
    }
  }, [menu])

  // Validate Menu Access
  if (
    !menu.children && !(menu?.privilege && checkAuthority(menu.privilege, 'get')) ||
    menu.children && !(menu.children.some(menuProps => menuProps.privilege && checkAuthority(menuProps.privilege, 'get')))
  ) return null;
  ...
  if (menuType === 'Header') {
    return (
      <React.Fragment>
        <Divider sx={{ pt: 2 }} />
        <StyledListSubheader sx={{ px: 1, mb: 1, pt: 2 }} color="primary">{menuName}</StyledListSubheader>
      </React.Fragment>
    );
    ...
    if (children) {
      return (
        <Box
          sx={{
            background: open ? Token.color.dashboardLight : Token.color.greyscaleGreyWhite,
            borderRadius: 3,
          }}
        >
    ...
    return (
      <StyledListItemButton
        selected={menuLink ? router.pathname === menuLink : false}
        onClick={() => handleClickMenu(menu)}
  ...
  return (
    <StyledListItemButton
      selected={router.pathname === menuLink}
      onClick={() => handleClickMenu(menu)}
      sx={{ borderRadius: 3 }}
    >
      {menuIcon && (
    ...
    <Stack direction="column">
      ...
      <List sx={{ px: 2 }}>
        <Stack direction="column" spacing={1}>
          {menuLists.map(menu => <MenuNested key={menu.menuName} {...menu} />)}
        </Stack>
      </List>
    </Stack>
  );
